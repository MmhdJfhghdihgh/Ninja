...
حقـوق للتـاريخ : @ALAPATH
@ALAPATH - 𓆩𓅃𝘼𝙇𝘼𝙋𝘼𝙏𝙃𓃠𓆪 كتـابـة الملـف :كتـابـة"""
...
ZelzalVP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𓆩𓅃𝘼𝙇𝘼𝙋𝘼𝙏𝙃𓃠𓆪 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/I_R_S_A_I) 𓆪\n\n"
    "**✾╎قائـمه اوامر تغييـر فـارات الصـور بأمـر واحـد فقـط - لـ اول مـره ع سـورس العابث يوزر بـوت 🦾 :** \n\n"
    ...
    "\n𓆩 [𓆩𓅃𝘼𝙇𝘼𝙋𝘼𝙏𝙃𓃠𓆪 - قنـاة الفـارات](t.me/I_R_S_A_I) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت العابث  يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
        ...
        if variable in heroku_var:
            ...
            await zed.edit("**✾╎تم اضافـة {} بنجـاح ☑️** \n**✾╎المضاف اليه :**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت العابث يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        heroku_var[variable] = vinfoيستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        heroku_var[variable] = vinfo
        ...
        if Config.HEROKU_APP_NAME is not None:
            ...
            return await ed(event, "✾╎اضبط Var المطلوب في Heroku على وظيفة هذا بشكل طبيعي `HEROKU_APP_NAME` اسم التطبيق اذا كنت لاتعلم.")
        heroku_var = app.config()**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت زد ثـون
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت العابث  يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    vmsr = "Africa/Cairo"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**✾╎جـاري أعـداد المنطقـه الزمنيـه لـ العابث 🌐...**" "
    if input_str == "العراق" or input_str == "عراق":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت العابث  يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
            await zed.edit("**✾╎تم اضـافـة المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المضـاف اليـه : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت زد العابث يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
